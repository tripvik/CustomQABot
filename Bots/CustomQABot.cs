    ...
    public class CustomQABot : ActivityHandler
    {
        ...
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            if (response.Length > 0)
            {
                ...
                if (response.FirstOrDefault().Context.Prompts.Any())
                {
...
//This method enumerates through prompts returned from QA services and adds a suggested card actions.
        protected Activity GetPromptOptions(QueryResult[] response, Activity reply)
        {
            ...
            foreach (var prompt in response[0].Context.Prompts.ToList())
            {
                Actions.Add(new CardAction() { Title = prompt.DisplayText, Type = ActionTypes.ImBack, Value = prompt.DisplayText });
            }
